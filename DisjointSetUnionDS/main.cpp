...
class DSUMethodsBase {
public:
#if __cplusplus >= 202002L
    static_assert(requires (NodeType* node) { { node->parent_ } -> std::convertible_to<NodeType*>; });
#endif

    // O(log*(n)) [log* - iterated logarithm, log*(n) = log2(log2(log2(...log2(n), until log2 > 0))), log*(65536) = 5, log*(2^65536) = 6]
    static constexpr NodeType* FindRoot(NodeType* node) noexcept {
...
class DisjointSetUnion : DSUMethodsBase<DSUTreeNode> {
public:
    ...
    // O(n)
    inline explicit DisjointSetUnion(size_t nodes_count) noexcept(false) : nodes_count_{nodes_count} {
        if (nodes_count) {
    ...
    // O(n)
    inline DisjointSetUnion(const DisjointSetUnion& other) noexcept(false) {
        *this = other;
    }

    // O(n)
    DisjointSetUnion& operator=(const DisjointSetUnion& other) noexcept(false) {
        if (this == &other) {
            return *this;
        }
        ...
        DSUTreeNode* const this_first_node = nodes_ = static_cast<DSUTreeNode*>(operator new(sizeof(DSUTreeNode) * count));
        const DSUTreeNode* const other_first_node = other.nodes_;
        const ptrdiff_t parent_offset = this_first_node - other_first_node;

        for (size_t i = 0; i < count; ++i) {
            const DSUTreeNode* other_i_node_parent = other_first_node[i].parent_;
            this_first_node[i].parent_ = other_i_node_parent ? const_cast<DSUTreeNode*>(other_i_node_parent + parent_offset) : nullptr;
            this_first_node[i].rank_ = other_first_node[i].rank_;
        }
    ...
    // O(1)
    inline constexpr DisjointSetUnion(DisjointSetUnion&& other) noexcept {
        *this = std::move(other);
    }

    // O(1)
    inline constexpr DisjointSetUnion& operator=(DisjointSetUnion&& other) noexcept {
        nodes_ = other.nodes_;
    ...
    // O(log*(n))
    inline constexpr bool Equivalent(size_t node_x_index, size_t node_y_index) const noexcept {
    ...
    // O(log*(n))
    constexpr void Unite(size_t node_x_index, size_t node_y_index) const noexcept {
        assert(node_x_index < nodes_count_ && node_y_index < nodes_count_);
        ...
        if (node_x_root_ptr == node_y_root_ptr) {
            // Do not unite already united nodes so that for each root node: root_node->parent_ == nullptr
            return;
        }
...
public:
    ...
    // O(n)
    inline explicit WeightedDisjointSetUnion(size_t nodes_count) noexcept(false) : nodes_count_{nodes_count} {
        if (nodes_count) {
            nodes_ = static_cast<WeightedDSUTreeNode*>(operator new(sizeof(WeightedDSUTreeNode) * nodes_count));
            std::memset(nodes_, 0, sizeof(WeightedDSUTreeNode) * nodes_count);
        }
    }

    // O(n)
    inline explicit WeightedDisjointSetUnion(const std::vector<int64_t>& weights) noexcept(false) : nodes_count_{weights.size()} {
        if (nodes_count_) {
            nodes_ = static_cast<WeightedDSUTreeNode*>(operator new(sizeof(WeightedDSUTreeNode) * nodes_count_));
            std::memset(nodes_, 0, sizeof(WeightedDSUTreeNode) * nodes_count_);
            WeightedDSUTreeNode* nodes_iter = nodes_;
            for (auto iter = weights.begin(), end = weights.end(); iter != end; ++iter, ++nodes_iter) {
    ...
    // O(n)
    inline WeightedDisjointSetUnion(const WeightedDisjointSetUnion& other) noexcept(false) {
        *this = other;
    }

    // O(n)
    WeightedDisjointSetUnion& operator=(const WeightedDisjointSetUnion& other) noexcept(false) {
        if (this == &other) {
            return *this;
        }
        ...
        WeightedDSUTreeNode* const this_first_node = nodes_ = static_cast<WeightedDSUTreeNode*>(operator new(sizeof(WeightedDSUTreeNode) * count));
        const WeightedDSUTreeNode* const other_first_node = other.nodes_;
        const ptrdiff_t parent_offset = this_first_node - other_first_node;

        for (size_t i = 0; i < count; ++i) {
            const WeightedDSUTreeNode* other_i_node_parent = other_first_node[i].parent_;
            this_first_node[i].parent_ = other_i_node_parent ? const_cast<WeightedDSUTreeNode*>(other_i_node_parent + parent_offset) : nullptr;
            this_first_node[i].rank_ = other_first_node[i].rank_;
            this_first_node[i].weight_ = other_first_node[i].weight_;
        }
    ...
    // O(1)
    inline constexpr WeightedDisjointSetUnion(WeightedDisjointSetUnion&& other) noexcept {
        *this = std::move(other);
    }

    // O(1)
    inline constexpr WeightedDisjointSetUnion& operator=(WeightedDisjointSetUnion&& other) noexcept {
        nodes_ = other.nodes_;
        nodes_count_ = other.nodes_count_;
        other.nodes_ = nullptr;
        other.nodes_count_ = 0;
    ...
    // O(log*(n))
    inline constexpr int64_t GetWeightInSet(size_t node_index) const noexcept {
        assert(node_index < nodes_count_);
        return FindRoot(&nodes_[node_index])->weight_;
    }

    // O(log*(n))
    inline constexpr void AddWeightInSet(size_t node_index, int64_t delta) {
        assert(node_index < nodes_count_);
        FindRoot(&nodes_[node_index])->weight_ += delta;
    }

    // O(log*(n))
    inline constexpr void SetWeightInSet(size_t node_index, int64_t weight) {
        assert(node_index < nodes_count_);
        FindRoot(&nodes_[node_index])->weight_ = weight;
    }

    // O(n)
    inline ~WeightedDisjointSetUnion() {
        operator delete (nodes_);
        nodes_ = nullptr;
        nodes_count_ = 0;
    }
