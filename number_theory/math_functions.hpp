...
ATTRIBUTE_CONST inline I128_CONSTEXPR uint64_t isqrt(uint128_t n) noexcept {
    ...
    uint128_t r_ = (n >> 6) + 16;
    uint64_t r =
        r_ > 0xFFFFFFFFFFFFFFFFull ? uint64_t(0xFFFFFFFFFFFFFFFFull) : uint64_t(r_);
    do {
        // m = (l + r + 1) / 2
...
/// @brief Modified version of __udivmodti4 from the clang
/// @param[in] a
/// @param[in] b
/// @return a % b
ATTRIBUTE_CONST constexpr uint64_t umod_128_64(uint128_t a, uint64_t b) noexcept {
    union utwords {
        uint128_t all;
        struct {
            ...
            uint64_t high;
            uint64_t low;
#endif  // CONFIG_BYTE_ORDER_LITTLE_ENDIAN
        } s;
    };
    ...
    // ---
    // 0 K
    ATTRIBUTE_ASSUME(n.s.high != 0);
    ATTRIBUTE_ASSUME(b != 0);
    uint32_t sr = 1 + n_udword_bits + uint32_t(math_functions::countl_zero(b)) -
                  uint32_t(math_functions::countl_zero(n.s.high));
    // 2 <= sr <= n_utword_bits - 1
    ATTRIBUTE_ASSUME(2 <= sr);
    ATTRIBUTE_ASSUME(sr <= n_utword_bits - 1);
    ...
    for (; sr > 0; --sr) {
        ...
        //      carry = 1;
        // }
        const int128_t s = int128_t(d.all - r.all - 1) >> (n_utword_bits - 1);
        carry = s & 1;
        r.all -= d.all & uint128_t(s);
    }
