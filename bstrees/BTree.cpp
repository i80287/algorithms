...
private:
    struct Node {
        ...
        constexpr size_t keys_size() const noexcept {
            return keys_size_;
        }

        constexpr int32_t first_key() const noexcept {
            return keys_[0];
        }
        ...
        void insert_checked(size_t pos, int32_t key, uint32_t child_index) noexcept {
...
void BTree::insert(int32_t key) {
    ...
    for (uint32_t child_index = 0;;) {
        ...
        assert(current_node->keys_size_ != 0 && current_node->keys_size_ <= keys_capacity_);
