...
namespace measure_tools {
    ...
    uint64_t MeasureMergeSort(const T* original_array, T* buffer, size_t length) {
        ...
        for (size_t i = MEASURE_LIMIT; i != 0; i--) {
            ...
            auto start = std::chrono::steady_clock::now();
            std::stable_sort(&buffer[0], &buffer[length]);
            auto end = std::chrono::steady_clock::now();
            std::chrono::nanoseconds delta = end - start;
            total_time += delta.count();
        ...
        uint64_t res = total_time / MEASURE_LIMIT;
        std::cout << "std::stable_sort merge sort average nanoseconds: " << res << '\n';
        return res;
    }
    ...
    template <typename T>
    requires std::is_arithmetic_v<T>
    uint64_t MeasureQuickSort(const T* original_array, T* buffer, size_t length) {
        uint64_t total_time = 0;
        ...
        for (size_t i = MEASURE_LIMIT; i != 0; i--) {
            ...
            auto start = std::chrono::steady_clock::now();
            QuickSort(&buffer[0], &buffer[length]);
            auto end = std::chrono::steady_clock::now();
            std::chrono::nanoseconds delta = end - start;
            total_time += delta.count();
...
int main(void) {
    ...
    for (size_t i = 0; i != TOTAL_TESTS; ++i) {
        ...
        case 0:
            ...
            quick_sort_time += measure_tools::MeasureQuickSort(array.get(), buffer_for_sorting.get(), length);
            break;
        case 1:
            quick_sort_time += measure_tools::MeasureQuickSort(array.get(), buffer_for_sorting.get(), length);
            qsort_time += measure_tools::MeasureQSort(array.get(), buffer_for_sorting.get(), length);
    ...
    std::cout
        ...
        << "\nstd::stable_sort merge sort average nanoseconds: " << merge_sort_time / TOTAL_TESTS
        << "\n::QuickSort quick sort average nanoseconds:      " << quick_sort_time / TOTAL_TESTS;
}
