...
std::vector<string_view> Search(string_view text, string_view query, size_t max_result_size) {
    ...
    size_t total_lines =
        act.FindAllSubstringsInTextAndCountLines</* IsExactWordsMatching = */ IsExactWordsMatching,
            ...
            [&query_words_on_lines](size_t line_number, size_t query_word_index) constexpr {
                bool new_line = query_words_on_lines.empty() ||
    ...
    double total_lines_log = std::log(static_cast<double>(total_lines));
    ...
    for (size_t line_index = 0; const LineInfo& line_info : query_words_on_lines) {
        double line_score  = 0;
        double words_count = static_cast<double>(line_info.WordsCount);
    ...
    size_t result_size = std::min(lines_score.size(), max_result_size);
    std::vector<string_view> result_lines(result_size);
    for (size_t result_index = 0; result_index < result_size; result_index++) {
        size_t line_index       = lines_score[result_index].LineIndex;
        size_t line_start_index = query_words_on_lines[line_index].LineStartIndex;
        size_t line_end_index   = query_words_on_lines[line_index].LineEndIndex;
